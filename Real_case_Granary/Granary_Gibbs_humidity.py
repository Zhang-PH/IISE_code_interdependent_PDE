...
# %%
class GibbsSampler:
    def __init__(self, Y1_obs, B_mat, Beta1, X_d_1, Y_d_1, Z_d_1, T_f_1, B_psi):
        self.Y1_obs = Y1_obs
        self.B_mat = B_mat
    ...
    def psi_1(self, res_t11, res_t12, res_t13, res_beta1, sigma_prop, gamma_0):  # sample psi_1
        ...
        for i in range(shape):
            ...
            square_cur = (t11_cur.T @ t11_cur + t12_cur.T @ t12_cur + t13_cur.T @ t13_cur) / (2 * sigma_psi)
            square_new = (next_t11.T @ next_t11 + t12_cur.T @ t12_cur + t13_cur.T @ t13_cur) / (2 * sigma_psi)
    ...
    def psi_2(self, res_t11, res_t12, res_t13, res_beta1, sigma_prop, gamma_0):  # sample psi_2
        ...
        for i in range(shape):
            ...
            square_cur = (t11_cur.T @ t11_cur + t12_cur.T @ t12_cur + t13_cur.T @ t13_cur) / (2 * sigma_psi)
            square_new = (t11_cur.T @ t11_cur + next_t12.T @ next_t12 + t13_cur.T @ t13_cur) / (2 * sigma_psi)
    ...
    def psi_3(self, res_t11, res_t12, res_t13, res_beta1, sigma_prop, gamma_0):  # sample psi_3
        ...
        for i in range(shape):
            ...
            square_cur = (t11_cur.T @ t11_cur + t12_cur.T @ t12_cur + t13_cur.T @ t13_cur) / (2 * sigma_psi)
            square_new = (t11_cur.T @ t11_cur + t12_cur.T @ t12_cur + next_t13.T @ next_t13) / (2 * sigma_psi)
            exp1 = (-square_cur - gamma_0 * zeta_sumsq_old / 2)
            exp2 = (-square_new - gamma_0 * zeta_sumsq_new / 2)
            if exp2 > exp1:
    ...
    def gibbs_burn(self, N_burn, n, K, a_e, b_e, a_0, b_0, sigma_prop, nt, shape):
        ...
        for i in range(N_burn):
            ...
            sample11 = self.psi_1(res_psi11[-1], res_psi12[-1], res_psi13[-1], res_beta1[-1], sigma_prop, res_gamma[-1])
            res_psi11.append(sample11)
            sample12 = self.psi_2(res_psi11[-1], res_psi12[-1], res_psi13[-1], res_beta1[-1], sigma_prop, res_gamma[-1])
            res_psi12.append(sample12)
            sample13 = self.psi_3(res_psi11[-1], res_psi12[-1], res_psi13[-1], res_beta1[-1], sigma_prop, res_gamma[-1])
        ...
        print("sigma_e_2 estimamation:", res_sigma[-1])
        print("gamma_0 estimamation:", res_gamma[-1])
        print("beta1 estimamation:", res_beta1[-1])

        print("psi_1 estimamation:", res_psi11[-1])
        print("psi_2 estimamation:", res_psi12[-1])
        print("psi_3 estimamation:", res_psi13[-1])
...
K = beta_ini_h.shape[0]  # basis function
a_e = 1.001
b_e = 0.001
a_0 = 0.001
b_0 = 0.001
...
GibbsObj_2 = GibbsSampler(Y_h, B_h, beta_ini_h, X_d_h, Y_d_h, Z_d_h, T_f_h, B_psi)
res_psi21, res_psi22, res_psi23, res_beta2, res_sigma2, res_gamma2, zeta_burn = GibbsObj_2.gibbs_burn(N_burn, n, K, a_e,
                                                                                                      b_e, a_0, b_0,
                                                                                                      sigma_prop1, nt,
                                                                                                      shape_psi)
end_time21 = time.time()
print("PDE humidity burn_in stage running time:", end_time21 - start_time21, "s")
# %%
# PDE2 steady stage
start_time22 = time.time()
res_psi21_normal, res_psi22_normal, res_psi23_normal, res_beta2_normal, res_sigma2_normal, res_gamma2_normal, zeta_normal = GibbsObj_2.gibbs_normal(
    N_normal, n, K, a_e, b_e, a_0, b_0, sigma_prop2, nt)
end_time22 = time.time()
print("PDE humidity steady_stage running time:", end_time22 - start_time22, "s")
# %%
np.save('output_hum/psi_1_burn.npy', res_psi21)
np.save('output_hum/psi_2_burn.npy', res_psi22)
np.save('output_hum/psi_3_burn.npy', res_psi23)
np.save('output_hum/beta_burn.npy', res_beta2)

np.save('output_hum/sigma_burn.npy', res_sigma2)
np.save('output_hum/gamma_burn.npy', res_gamma2)

np.save('output_hum/psi_1_steady.npy', res_psi21_normal)
np.save('output_hum/psi_2_steady.npy', res_psi22_normal)
np.save('output_hum/psi_3_steady.npy', res_psi23_normal)
np.save('output_hum/beta_steady.npy', res_beta2_normal)

np.save('output_hum/sigma_steady.npy', res_sigma2_normal)
np.save('output_hum/gamma_steady.npy', res_gamma2_normal)

np.save('output_hum/zeta_burn.npy', zeta_burn)
np.save('output_hum/zeta_steady.npy', zeta_normal)
# %%
np.save('output_hum/sigma_sample.npy', GibbsObj_2.sigma_set)
np.save('output_hum/gamma_sample.npy', GibbsObj_2.gamma_set)
np.save('output_hum/psi21_sample.npy', GibbsObj_2.psi11_set)
np.save('output_hum/psi22_sample.npy', GibbsObj_2.psi12_set)
np.save('output_hum/psi23_sample.npy', GibbsObj_2.psi13_set)
# %%
# 标准差
for i in range(shape_psi):
    print('psi1', np.std(np.array(res_psi21_normal)[:, i]))
    print('psi2', np.std(np.array(res_psi22_normal)[:, i]))
    print('psi3', np.std(np.array(res_psi23_normal)[:, i]))
# %%
...
# %%
sigma_hat = res_sigma2_normal[-1]
gamma_hat = res_gamma2_normal[-1]
# %%
# PDE error
zeta_ob2 = GibbsObj_2.zeta_cal(psi_21_hat, psi_22_hat, psi_23_hat, T_f_h, X_d_h, Y_d_h, Z_d_h, beta2_hat)

zeta_ob_ss = zeta_ob2.T @ zeta_ob2
...
print('rmse_zeta', np.sqrt(zeta_ob_ss / zeta_ob2.shape[0]))
print('SST', zeta_ob_ss)
# %%
# Bspiline error
error_bs2 = Y_h - B_h @ beta2_hat

error_ss = error_bs2.T @ error_bs2

print('mean_BSpline', np.mean(error_bs2))
print('mean_abs_BSpline', np.mean(np.abs(error_bs2)))
print('max_BSpline', np.max(error_bs2))
print('min_BSpline', np.min(error_bs2))
print('rmse_BSpline', np.sqrt(error_ss / error_bs2.shape[0]))
...
wb = Workbook()
dest_filename = "%s_Humidity_Time_varying_1by1_result.xlsx" % (now.strftime('%Y%m%d-%H%M%S'))
...
ws.append(["psi1", '采样估计值'] + [psi_21_hat[i] for i in range(shape_psi)])
ws.append(["psi2", '采样估计值'] + [psi_22_hat[i] for i in range(shape_psi)])
ws.append(["psi3", '采样估计值'] + [psi_23_hat[i] for i in range(shape_psi)])

ws.append(["sigma_2", '采样估计值', sigma_hat, (b_e + error_bs2.T @ error_bs2 / 2) / (a_e + n / 2 - 1)])
ws.append(["gamma", '采样估计值', gamma_hat, (a_0 + K / 2) / (b_0 + zeta_ob2.T @ zeta_ob2 / 2)])
...
O = ['Value', 'Humidity_Time_Varying', '3', '3', n, K, a_e, b_e, a_0, b_0, sigma_psi, sigma_prop1, sigma_prop2, nt,
     N_burn, N_normal]
