...
if __name__ == '__main__':
    ...
    # vector data
    temp_4d = np.zeros(tp * zp * yp * xp)
    for l in range(tp):
        for k in range(zp):
            for j in range(yp):
                for i in range(xp):
                    temp_4d[tp * (zp * (yp * i + j) + k) + l] = grid_4d[i, j, k, l]
#     print(grid_4d)
    ...
    # Bspline
    bsp_4d = Bspline()
    bsp_4d.fit4D(t, x, y, z, temp_4d, 3, nodeVector_t, nodeVector_x, nodeVector_y, nodeVector_z)
    beta_4 = bsp_4d.beta()
    # print('beta_4', beta_4)
...
# temp_estimation
temp_estimation = bsp_4d.predict_4D(tt, x, y, z)
print('n:', temp_estimation.shape)
temp_pre_restore = np.zeros([xp, yp, zp, new_tp])#unknown time point
# temp_pre_restore: temp_estimation to 4-dim array
for i in range(xp):
    for j in range(yp):
        for k in range(zp):
            for l in range(new_tp):
                temp_pre_restore[i, j, k, l]= temp_estimation[new_tp * (zp * (yp * i + j) + k) + l]

# validation_origin4-dim，temp_validation:1-dim
validation_origin = boundary[:, :, :, :(Max_t+1):rhot_new]
temp_validation = np.zeros(xp*yp*zp*new_tp)
for l in range(new_tp):
        for k in range(zp):
            for j in range(yp):
                for i in range(xp):
                    temp_validation[new_tp * (zp * (yp * i + j) + k) + l] = validation_origin[i, j, k, l]

#Bspline error
error_bs1 = temp_estimation - temp_validation
...
plt.tight_layout()
plt.savefig('pic/Temperature_Profile_%s'%now.strftime('%Y%m%d'), dpi=1200)
...
t_theta = np.linspace(0, Max_t, new_tp)
arr1 = np.array([t_theta.min()] * (3))
arr2 = np.array([t_theta.max()] * (3))
n1 = np.array([240, 480])
new_nodeVector_t = np.concatenate((arr1, n1))
new_nodeVector_t = np.concatenate((new_nodeVector_t, arr2))

B_theta = bsp_4d.build_spline_mat(t_theta, 5-3, 3, new_nodeVector_t)

tnew = np.linspace(0, Max_t, 2)
xx = np.linspace(0, Max_x, 100)
yy = np.linspace(0, Max_y, 100)
zz = np.linspace(0, Max_z, 79)
temp_pre_new = bsp_4d.predict_4D(tnew, xx, yy, zz)

restore2 = np.zeros([100, 100, 79, 2])#预测位置时刻的结果         
for i in range(100):
    for j in range(100):
        for k in range(79):
            for l in range(2):
                restore2[i, j, k, l]= temp_pre_new[2 * (79 * (100 * i + j) + k) + l]
...
color = [plt.get_cmap("jet", 100)(int(float(i-min_v)/(max_v-min_v)*100)) for i in v_1d]
...
fig.colorbar(im, format=matplotlib.ticker.FuncFormatter(lambda x,pos:int(x*(max_v-min_v)+min_v)),label='Temperature/℃')
ax.set_xlabel('x')
ax.set_ylabel('y')
ax.set_zlabel('z')
...
plt.savefig('pic/initial-temp-720-axis.png', dpi=1200)
plt.show()