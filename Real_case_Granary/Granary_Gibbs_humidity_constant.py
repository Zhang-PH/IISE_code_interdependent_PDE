...
K = beta_ini_h.shape[0]  # number of basis functions
a_e = 0.001
b_e = 0.001
a_0 = 0.001
b_0 = 0.001
...
GibbsObj_2 = GibbsSampler(Y_h, B_h, beta_ini_h, X_d_h, Y_d_h, Z_d_h, T_f_h)
...
# %%
np.save('output_hum_constant/psi_1_burn.npy', res_psi21)
np.save('output_hum_constant/psi_2_burn.npy', res_psi22)
np.save('output_hum_constant/psi_3_burn.npy', res_psi23)
np.save('output_hum_constant/beta_burn.npy', res_beta2)

np.save('output_hum_constant/sigma_burn.npy', res_sigma2)
np.save('output_hum_constant/gamma_burn.npy', res_gamma2)

np.save('output_hum_constant/psi_1_steady.npy', res_psi21_normal)
np.save('output_hum_constant/psi_2_steady.npy', res_psi22_normal)
np.save('output_hum_constant/psi_3_steady.npy', res_psi23_normal)
np.save('output_hum_constant/beta_steady.npy', res_beta2_normal)

np.save('output_hum_constant/sigma_steady.npy', res_sigma2_normal)
np.save('output_hum_constant/gamma_steady.npy', res_gamma2_normal)
# %%
...
wb = Workbook()
dest_filename = "%s_Humidity_Constant_result.xlsx" % (now.strftime('%Y%m%d-%H%M%S'))
...
ws.append(["psi1", '采样估计值'] + [psi_21_hat for i in range(shape_psi)])
ws.append(["psi2", '采样估计值'] + [psi_22_hat for i in range(shape_psi)])
ws.append(["psi3", '采样估计值'] + [psi_23_hat for i in range(shape_psi)])
...
O = ['Value', 'Humidity_Constant', '3', '3', n, K, a_e, b_e, a_0, b_0, sigma_psi, sigma_prop1, sigma_prop2, nt, N_burn,
     N_normal]
